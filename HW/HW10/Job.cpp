...
std::ostream &operator<<(std::ostream &out, const Job *j) {
  ...
  strftime(time_buff, 80, " time: %c %Z",
           timeinfo); // Converts timeinfo into a C-string
...
void Job::removeFromUrgent() { priority_ptr->remove_from_index(urgent_hook); }

// Removes the job from whichever TimeQueue it belongs to
void Job::removeFromTime() { time_ptr->remove_from_index(time_hook); }

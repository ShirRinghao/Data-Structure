...
void TimeQueue::percolate_up() {
  ...
  for (unsigned int child = heap.size() - 1; child > 0;) {
    ...
    // If parent is bigger than parent, swap them, then update their new hook
    if (heap[child]->getTime() < heap[parent]->getTime()) {
      Job *tmp = heap[child];
...
void TimeQueue::percolate_down(int parent) {
  ...
  while ((parent * 2) + 1 < (int)heap.size()) {
    ...
    if (heap[swap]->getTime() > heap[left_child]->getTime()) {
      swap = left_child;
    }
    // Then check the right child
    if (right_child < (int)heap.size() &&
        heap[swap]->getTime() > heap[right_child]->getTime()) {
      swap = right_child;
    }
...
void TimeQueue::push(Job *job) {
  ...
  heap[heap.size() - 1]->time_hook = heap.size() - 1;
  job->time_ptr = this;
...
void TimeQueue::printHeap(std::ostream &out) {
  out << "TQ " << heap.size() << " jobs:" << std::endl;

...
bool is_leaf(Node *p) { return p && !(p->left) && !(p->right); }
...
rope_iterator::rope_iterator() { ptr_ = NULL; }

rope_iterator::rope_iterator(Node *p) { ptr_ = p; }

int rope_iterator::weight() const { return ptr_->weight; }

const std::string &rope_iterator::value() const { return ptr_->value; }
...
rope_iterator Rope::end() const { return rope_iterator(NULL); }

int Rope::size() const { return size_; }

void Rope::print(std::ostream &out) const { print(out, root, 0); }
...
Node *Rope::expose_root() const { return root; }

...
void usage() {
  std::cerr << "Usage: <executable_name> <data structure> <operation> <input "
               "file> <output file>"
            << std::endl;
  std::cerr << "Usage: <executable_name> <data structure> <operation> random "
               "<input count> <string length> <output file>"
            << std::endl;
  std::cerr << "Usage:    <data structure> = vector, list, bst, "
               "priority_queue, hash_table"
            << std::endl;
  std::cerr << "Usage:    <operation>      = sort, remove_dups_same_order, "
               "remove_dups_any_order, mode"
            << std::endl;
  exit(1);
}
...
void load_data(const std::string &input, std::string *&input_data,
               int &input_count, int string_length) {
...
int main(int argc, char *argv[]) {
  ...
  if (data_structure == "vector")
    ...
    priority_queue_test(input_data, input_count, operation, output_data,
                        output_count);
  else if (data_structure == "hash_table") // STL unordered_set or unordered_map
    hash_table_test(input_data, input_count, operation, output_data,
                    output_count);
  else {
    std::cerr << "Error: unknown data structure: " << data_structure
              << std::endl;
    usage();
    exit(0);
  }
  ...
  double operation_time =
      double(after_operation - before_operation) / CLOCKS_PER_SEC;
  ...
  std::cout << "input/output ratio: " << input_count << ":" << output_count
            << std::endl;
  return 0;
}

...
void read_grid(istream &istr, vector<vector<bool>> &blocked_grid, int &start_x,
               int &start_y) {
...
void print_grid(vector<vector<bool>> const &blocked_grid, int start_x,
                int start_y, const vector<Point> &path, const bool success) {

  cout << "Here is the grid with the origin in the upper left corner, x "
          "increasing \n"
       << "horizontally and y increasing down the screen.  An 'X' represents a "
          "blocked\n"
...
void all_paths(int x, int y, const vector<vector<bool>> &blocked_grid,
               int &total) {
  if (x == 0 && y == 0) {
    total += 1;
    return;
  }
  if (blocked_grid[x][y] == true) {
    return;
  }
  if (x - 1 >= 0) {
    all_paths(x - 1, y, blocked_grid, total);
  }
  if (y - 1 >= 0) {
    all_paths(x, y - 1, blocked_grid, total);
  }
}

void check3(int x, int y, const vector<vector<bool>> &blocked_grid,
            vector<Point> &path, bool &find) {
  if (x == 0 && y == 0) {
    find = true;
    return;
  }
  if (blocked_grid[x][y] != true && x - 1 >= 0) {
    check3(x - 1, y, blocked_grid, path, find);
    if (find == true) {
      path.push_back(Point(x, y));
    }
  }
  if (y - 1 >= 0 && blocked_grid[x][y - 1] != true && find != true) {
    check3(x, y - 1, blocked_grid, path, find);
    if (find == true) {
      path.push_back(Point(x, y));
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  all_paths(start_x, start_y, blocked_grid, total);
  cout << total << " paths are found" << endl;
  bool find = false;
  check3(start_x, start_y, blocked_grid, path, find);
  print_grid(blocked_grid, start_x, start_y, path, find);
  for (unsigned int i = 0; i < path.size(); i++) {

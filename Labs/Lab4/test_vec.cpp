...
int main() {
  ...
  v3 = v2;
  v3.clear();
  cout << "\nAfter copying v2 to v3 and clearing v3,  v2.size() = " << v2.size()
       << " and v3.size() = " << v3.size() << endl;
  ...
  // test resize
  v1.resize(20, 100);
  cout << "\nNow testing resize.  Resizing v1 to have 20 elements and v2 to "
          "have 2 elements\n";
  ...
  int a = remove_matching_elements(test_vector, 1);
  std::cout << "vector now contains " << test_vector.size() << " elements"
            << std::endl;
  for (unsigned int i = 0; i < test_vector.size(); i++) {
    std::cout << i << ": " << test_vector[i] << ' ';
  }
  std::cout << std::endl;
  std::cout << a << " elements are removed from vector" << std::endl;
  return 0;
}

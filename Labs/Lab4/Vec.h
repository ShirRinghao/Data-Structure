...
// class is implemented using a dynamically allocated array (of templated type
// T). We ensure that that m_size is always <= m_alloc and when a push_back or
// resize call would violate this condition, the data is copied to a larger
// array.
...
// If n is less than or equal to the current size, just change the size.  If n
// is greater than the current size, the new slots must be filled in with the
// given value. Re-allocation should occur only if necessary.  push_back should
// not be used.
...
template <class T>
int remove_matching_elements(Vec<T> &vector, const T &element) {
  int removed = 0;

...
private:
  ...
  TreeNode<T> *copy_helper(TreeNode<T> *old_root, TreeNode<T> *curr) {
    if (!old_root) {
      return old_root;
    }
    if (old_root->right) {
      TreeNode<T> *tmp = new TreeNode<T>(old_root->right->value);
      curr->right = tmp;
      copy_helper(old_root->right, curr->right);
    }
    if (old_root->left) {
      TreeNode<T> *tmp = new TreeNode<T>(old_root->left->value);
      curr->left = tmp;
  ...
  int erase(T const &key_value,
            TreeNode<T> *&p) { /* Implemented in Lecture 18 */
  }
